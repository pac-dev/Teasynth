 ...
 */
 export class Faust2MD {
     static REGEX_BEG_TITLE = /^\s*\/\/#{3,}\s*([^#]*[^#\s])\s*#{3,}$/
     static REGEX_END_TITLE = /^\s*((\/\/#{3,})|(\s*))$/
     static REGEX_BEG_SECTION = /^\s*\/\/={3,}\s*([^=]*[^=\s])\s*={3,}$/
     static REGEX_END_SECTION = /^\s*((\/\/={3,})|(\s*))$/
     static REGEX_BEG_COMMENT = /^\s*\/\/-{3,}\s*([^-]*[^=\s])\s*-{3,}$/
     static REGEX_END_COMMENT = /^\s*((\/\/-{3,})|(\s*))$/
     static REGEX_INDENT = /(^\s*\/\/\s*)[^\s]/
     static REGEX_COMMENT = /^\s*\/\//
    /**
    ...
    static parse(strIn, fileName, optionsIn) {
        const options = { tabsize: 4, code: false, front: false, ...optionsIn };
        let strOut = "";
